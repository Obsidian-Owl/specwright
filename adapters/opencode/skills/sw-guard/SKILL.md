...
- Existing opencode.json settings, git hooks, CI workflows
...
- `opencode.json` -- Opencode session-level permissions and plugin event hooks
...
- Read dependency manifests, git hooks, opencode.json settings, CI/CD workflows. Don't guess — read files.
...
- Organize by layer: session (Opencode plugin events), commit (pre-commit), push (pre-push), CI/CD (GitHub Actions). Each independently approvable.
- Session layer: recommend Opencode plugin event hooks using detected tools. Generate inline shell commands from config.json — never hardcode tool names. User chooses destination: `opencode.json` (project-level). Read existing hooks first, show diff, merge (don't overwrite), detect duplicates.
