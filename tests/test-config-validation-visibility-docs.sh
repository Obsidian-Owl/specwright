...
assert_contains "$INIT_SKILL" "runtime mode as an explicit Git policy choice" "sw-init treats runtime mode as an explicit Git policy choice"
assert_contains "$INIT_SKILL" "recommend \`project-visible\` for Claude-oriented installs" "sw-init recommends project-visible for Claude-oriented installs"
assert_contains "$INIT_SKILL" "separately from tracked work-artifact publication" "sw-init keeps runtime mode separate from tracked work-artifact publication"
...
assert_contains "$DOCTOR_SKILL" "Claude-oriented installs that remain on \`git-admin\`" "sw-doctor warns when Claude-oriented installs stay on git-admin"
assert_contains "$DOCTOR_SKILL" "config.git.runtime.projectVisibleRoot" "sw-doctor names the projectVisibleRoot config key when runtime roots are unsafe"
assert_contains "$DOCTOR_SKILL" "tracked project artifacts or \`.git\`-mirrored paths" "sw-doctor rejects project-visible roots that overlap tracked artifacts or git mirrors"
assert_contains "$DOCTOR_SKILL" "authoritative runtime roots" "sw-doctor reports the authoritative runtime roots clearly"
