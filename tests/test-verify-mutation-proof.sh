...
if [ -f "$SOURCE_VERIFY" ]; then
  ...
  assert_contains "$SOURCE_VERIFY" "record or refresh the" "source sw-verify records accepted-mutant lineage"
  assert_contains "$SOURCE_VERIFY" "90 days from approval" "source sw-verify documents accepted-mutant expiry"
  ...
  info "skipping source verify content checks for missing ${SOURCE_VERIFY#"$ROOT_DIR"/}"
fi

echo ""
echo "--- Packaged Claude Code docs ---"
if [ -f "$ADAPTER_CLAUDE" ]; then
  ...
  info "skipping adapter CLAUDE content checks for missing ${ADAPTER_CLAUDE#"$ROOT_DIR"/}"
fi
...
if [ -f "$DIST_CLAUDE" ]; then
  ...
  info "skipping dist CLAUDE content checks for missing ${DIST_CLAUDE#"$ROOT_DIR"/}"
fi

echo ""
echo "--- Built mutation surfaces ---"
if [ -f "$DIST_VERIFY" ]; then
  assert_contains "$DIST_VERIFY" "--accept-mutant {id}" "dist sw-verify preserves accepted-mutant CLI"
  assert_contains "$DIST_VERIFY" "record or refresh the" "dist sw-verify preserves accepted-mutant recording behavior"
  assert_contains "$DIST_VERIFY" "90 days from approval" "dist sw-verify preserves accepted-mutant expiry"
  ...
  info "skipping dist sw-verify content checks for missing ${DIST_VERIFY#"$ROOT_DIR"/}"
fi

if [ -f "$DIST_GATE_TESTS" ]; then
  ...
  info "skipping dist gate-tests content checks for missing ${DIST_GATE_TESTS#"$ROOT_DIR"/}"
fi

if [ -f "$DIST_TESTER" ]; then
  assert_contains "$DIST_TESTER" "silently skipping mutation review" "dist tester preserves T3 fallback wording"
else
  info "skipping dist tester content checks for missing ${DIST_TESTER#"$ROOT_DIR"/}"
fi
