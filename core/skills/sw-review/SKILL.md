...
- Read `config.git.prTool` before invoking `gh`; this determines which CLI is
  preferred. Default to `gh` if unset.
...
- Cap fetched comments at 50 per type using `--per-page 50`. If the response
  indicates more exist, display a truncation notice so the user knows the list
  is incomplete.
...
- To post an issue comment reply, POST to
...
- **Rate limit exceeded** — `gh api` returns HTTP 403 or 429. Surface the error
  message and the `X-RateLimit-Reset` time if available. Ask the user to retry
